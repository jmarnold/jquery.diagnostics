...
(function($) {
    ...
    function diagnosticsView(options) { 
        this.element = $('<div class="diagnostics"><h2 class="title">' + options.title + '</h2></div>');
        this.element.css({
            left: '4%',
            width: '90%',
            top: '5%',
        ...
        activate: function() {
            ...
            dashboardView.configure(function() {
                ...
                for(var i = 0; i < topics.length; i++) {
                    ...
                    var item = $('<li><a href="#">' + topic + '</a></li>');
                    $('a', item).attr('style', 'color:white').click(function(e) {
                        e.preventDefault();
                        $.diagnostics.switchTo(topic);
                    });
                    item.appendTo(list);
                }
                
                if( $('li', list).size() != 0 ) {
                    this.append('<h4>Available screens</h4>');
                    this.append(list);
                }
                else {
                    this.append('<p>No screens registered</p>');
                }
            });
            dashboardView.show();
        },
